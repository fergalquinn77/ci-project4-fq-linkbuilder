...
class TestViews(TestCase):
    ...
    # Test can user display tickets without login
    ...
    def test_add_support_ticket_with_login(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response,
                                template_name='accounts/add_ticket.html')
        response2 = self.client.post(f'/add-ticket/',
    ...
    def test_ticket_details(self):
        user = User.objects.create_user('foo', 'myemail@test.com', 'bar')
        self.client.login(username='foo', password='bar')
        self.client.post(f'/add-ticket/', {'title': 'Test New Query',
                         'query': 'Test message'})
        ticket = Support_Tickets.objects.get(title='Test New Query')
        response = self.client.get(f'/ticket-details/{ticket.id}/')
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response,
    ...
    def test_toggle_ticket(self):
        ...
        response = self.client.get(f'/toggle-ticket-status/{ticket.id}/')
    ...
    def test_post_message(self):
        ...
        self.client.post(f'/add-ticket/', {'title': 'Test New Query',
                         'query': 'Test message'})
        ticket = Support_Tickets.objects.get(title='Test New Query')
        response = self.client.post(f'/ticket-details/{ticket.id}/',
    ...
    def test_profile_creation(self):
        ...
        self.assertEqual(user_profile.company_name, 'Test Company')
