...
class TestViews(TestCase):
    ...
    def test_external_page(self):
        test_user = get_user_model().objects.create(username="foo",
                                                    email="test@test.com",
                                                    password="bar")
    ...
    def test_edit_link_page(self):
        ...
        link = Url_Links.objects.create(title='Test Title',
                                        link='http://www.rte.ie', user=user)
    ...
    def test_can_edit_link_page(self):
        ...
        link = Url_Links.objects.create(title='Test Title',
                                        link='http://www.rte.ie', user=user)
        response = self.client.post(f'/edit/{link.id}/',
                                    {'title': 'Changed Title',
    ...
    def test_can_add_link(self):
        ...
        response = self.client.post(f'/add/', {'title': 'Test New Link',
                                    'link': 'http://www.rte.ie'})
    ...
    def test_can_add_link(self):
        ...
        number_links_before = Url_Links.objects.count()
        response = self.client.post(f'/add/', {'title': 'Test New Link',
                                    'link': 'http://www.rrrrte.ie'})
        number_links_after = Url_Links.objects.count()
        self.assertEqual(number_links_before, number_links_after)

    # Test Edit Link with invalid URL
    def test_can_edit_link_page(self):
        user = User.objects.create_user('foo', 'myemail@test.com', 'bar')
        self.client.login(username='foo', password='bar')
        link = Url_Links.objects.create(title='Test Title',
        ...
        response = self.client.post(f'/edit/{link.id}/',
                                    {'title': 'Changed Title',
                                        'link': 'http://www.rteeeeee.ie'})
        updated_link = Url_Links.objects.get(id=link.id)
        self.assertNotEqual(updated_link.title, 'Changed Title')
